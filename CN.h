...
public:
    ...
    */
    double meanSecondElement(const std::vector<std::pair<double, double>>& vec) { //calculate the mean of the input of interest rates
        ...
        double sum = std::accumulate(vec.begin(), vec.end(), 0.0,
            [](double acc, const std::pair<int, int>& p) {
                return acc + p.second;
            });
        return sum / vec.size();
    }
    ...
    */
    std::vector<std::vector<double>> solve(double dP = 0, double ds_ = 0, double dr_ = 0) {
        ...
        //Coefficient for the CN scheme
        std::vector<double> alpha(Nx + 1), beta(Nx + 1), gamma(Nx + 1);


        //Tridiagonal matrix component
        std::vector<double> lower(Nx - 1), main(Nx - 1), upper(Nx - 1);
        ...
        for (int t = Nt - 1; t >= 0; --t) {
            ...
            b[0] = alpha[1] * (V[0][t] + V[0][t + 1]) - alpha[0] * V[0][t + 1];
            b[Nx - 2] = gamma[Nx - 1] * (V[Nx][t] + V[Nx][t + 1]);

            for (int i = 1; i < Nx; ++i) {
                b[i - 1] += (1.0 + beta[i]) * V[i][t + 1];
            }
            ...
            //Update and account for American Options
            subvec.clear();
            std::copy(R.begin(), R.begin() + t, std::back_inserter(subvec));
            for (int i = 1; i < Nx; i++) {
                V[i][t] = solution[i - 1];
    ...
    */
    std::string validate() { 
        std::vector<double> d1(Nx + 1);
        std::vector<double> d2(Nx + 1);
        ...
        std::vector<double> S(Nx + 1); //price today
        for (int i = 0; i <= Nx; ++i) {
            S[i] = i * dS;
        ...
        for (int i = 0; i <= Nx; ++i) {
            ...
            flags = (std::fabs(V[i][Nt] - target[i][Nt]) > std::max(0.01*V[i][Nt], 1e-9)) ? flags + 1 : flags;
        }
        //today payoff
        for (int i = 0; i <= Nx; ++i) {
            V[i][0] = isCall ? std::max(S[i] - K, 0.0) : std::max(K - S[i], 0.0);
            mean += std::fabs(V[i][0] - target[i][0]);
            flags = (std::fabs(V[i][0] - target[i][0]) > std::max(0.05 * V[i][0], 1e-9)) ? flags + 1 : flags;
        }
        ...
        for (int t = Nt - 1; t >= 1; --t) {
            ...
            for (int i = 0; i <= Nx; i++) {
                d1[i] = (1 / (sigma * sqrt(T-time))) * (std::log(S[i] / K) + trapezoidal(subvec, time, T) + (sigma / 2) * (T-time));
                d2[i] = d1[i] - sigma * sqrt(T-time);
                V[i][t] = isCall ? normCdf(d1[i]) * S[i] - normCdf(d2[i]) * K * std::exp(-trapezoidal(subvec, time,T)) : normCdf(-d2[i]) * K * std::exp(-trapezoidal(subvec, time, T)) - normCdf(-d1[i]) * S[i];
                V[i][t] *= std::exp(-trapezoidal(subvec_1, 0, time));
                //We can include the if clause below but the payoff will never supercede the value
                if (isAmerican) {
                    V[i][t] = std::max(V[i][t], isCall ? std::max(S[i] - K, 0.0) * std::exp(-trapezoidal(subvec_1, 0, time)) : std::max(K - S[i], 0.0) * std::exp(-trapezoidal(subvec_1, 0, time)));
                }
                mean += std::fabs(V[i][t] - target[i][t]);
                flags = (std::fabs(V[i][t] - target[i][t]) > std::max(0.05 * V[i][t], 1e-9)) ? flags + 1 : flags;
            }
        }
        subvec_1.clear();
        mean /= (Nx + 1) * (Nt + 1);
        std::ostringstream oss;
        oss << "Mean Absolute Error for every grid point is: ";
        oss << std::fixed << std::setprecision(4) << mean;
        oss << "Points with more than 1% MAE: ";
        oss << std::fixed << flags;
